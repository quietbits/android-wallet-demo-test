...
import com.example.androidwalletdemo.AppWalletSigner
import com.example.androidwalletdemo.component.PageLayout
import com.example.androidwalletdemo.horizonUrl
import com.example.androidwalletdemo.networkPassphrase
...
fun InteractiveFlow(navController: NavHostController) {
  ...
  PageLayout("Interactive deposit and withdrawal", navController) {
    ...
    val (userStellarAddress, setUserStellarAddress) =
      remember { mutableStateOf("GAMQTINWD3YPP3GLTQZ4M6FKCCSRGROQLIIRVECIFC6VEGL5F64CND22") }
    ...
    val (userSecretKey, setUserSecretKey) =
      remember { mutableStateOf("SAICIEF6C7ENEPUCFYU5YE7CF6Z2BXUCYQ4MNB3RK3MMZ4GECYFJBWZ3") }
    val (assetCode, setAssetCode) = remember { mutableStateOf("SRT") }
    val (assetIssuer, setAssetIssuer) =
      remember { mutableStateOf("GCDNJUBQSX7AJWLJACMJ7I4BC3Z47BQUTMHEICZLE6MU4KQBRYG5JY6B") }
      ...
      AndroidView(
        factory = {
          WebView(pageContext).apply {
            ...
            // TODO: see if we can remove UnsafeWebViewClient (breaks for API 21)
